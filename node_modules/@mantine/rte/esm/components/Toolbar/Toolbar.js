...
function Toolbar(_a) {
  ...
  const { classes, cx } = useStyles({ sticky, stickyOffset }, { classNames, styles, name: "RichTextEditor" });
  const groups = controls.map((group, index) => {
    const items = group.filter((item) => CONTROLS[item]).map((item) => {
      ...
      return /* @__PURE__ */ React.createElement(ToolbarButton, {
        ...
        key: item,
        title: labels[item],
        noActive: CONTROLS[item].noActive
